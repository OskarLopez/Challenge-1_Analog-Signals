...
bool datos_procesados = false;  // Para controlar si los datos ya fueron procesados
...
void setup() {
  ...
  Serial.begin(9600);  // Configurar el serial para graficar en el Serial Plotter
...
void loop() {
  ...
  if (digitalRead(pulsadorInicio) == HIGH && !adquiriendo && !datos_procesados) {
    lcd.clear();
    lcd.setCursor(0, 0);
    ...
    datos_procesados = false;  // Reiniciar el estado de procesamiento
  }

  // Leer el estado del pulsador para detener la adquisición en cualquier momento
  if (digitalRead(pulsadorMostrar) == HIGH && adquiriendo) {
    adquiriendo = false;  // Detener la adquisición cuando se presione el pulsador
    procesarDatos();  // Procesar los datos capturados
    mostrarResultados();  // Mostrar resultados
    datos_procesados = true;  // Indicar que los datos han sido procesados
  }
  ...
  if (adquiriendo) {
    ...
    // Mostrar los datos de la señal en el Serial Plotter durante la adquisición
    ...
    // Si se llena el buffer, detener la adquisición
  ...
  // Permitir volver a adquirir datos presionando el botón 1 de nuevo
  if (digitalRead(pulsadorInicio) == HIGH && datos_procesados && !adquiriendo) {
    // Reiniciar el proceso para una nueva adquisición
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Adquiriendo...");
    
    muestras_capturadas = 0;  // Reiniciar el contador de muestras
    adquiriendo = true;  // Reiniciar la adquisición
    datos_procesados = false;  // Resetear el estado de procesamiento
  }

  // Implementar un pequeño delay para no sobrecargar el microcontrolador
  delay(10);
}

// Función para capturar las muestras de la señal y mostrarlas en el Serial Plotter
void capturarMuestras() {
  if (muestras_capturadas < buffer_size) {
    buffer[muestras_capturadas] = analogRead(A0);  // Capturar la señal del generador
    Serial.println(buffer[muestras_capturadas]);  // Enviar los datos al Serial Plotter
...
void procesarDatos() {
  ...
  // Calcular frecuencia y amplitud usando las muestras capturadas hasta el momento
  for (int i = 0; i < muestras_capturadas; i++) {
